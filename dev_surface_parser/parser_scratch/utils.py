...
def read_txt(path: str) -> list:
    ...
    return  data #[x.strip('\t') for x in data]
...
def create_del_list(inverted_stats_names: dict, valid_categories: list):
    ...
    Args:
        inverted_stats_names (dict): _description_
        valid_categories (list): _description_
    """
    del_list = [None] * (len(inverted_stats_names) - len(valid_categories))
    counter = 0
    for idx, (k, v) in enumerate(inverted_stats_names.items()):
        
        if k not in valid_categories and v != None:
            del_list[counter] = v
            counter += 1
